...
public static class StorageService
{
    ...
    private static DriveService GetDriveService()
    {
        ...
        lock (_driveLock)
        {
            ...
            // Prefer an explicit .env override if present, otherwise fall back
            // to a path relative to wherever the app is running from — this
            // makes the secrets folder portable across machines without
            // needing a per-machine .env edit.
            var clientJsonPath = Environment.GetEnvironmentVariable("GOOGLE_OAUTH_CLIENT_JSON")
                ?? Path.Combine(AppContext.BaseDirectory, "secrets", "oauth-client.json");

            if (!System.IO.File.Exists(clientJsonPath))
            throw new InvalidOperationException(
                $"Google OAuth client file not found at: {clientJsonPath}\n" +
                "Place oauth-client.json in a 'secrets' folder next to LLOIS.exe, " +
                "or set GOOGLE_OAUTH_CLIENT_JSON in .env to override.");
